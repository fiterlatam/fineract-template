...
public class Money implements Comparable<Money> {
    ...
    public static BigDecimal roundToMultiplesOf(final BigDecimal existingVal, final Integer inMultiplesOf) {
        ...
        if (inMultiplesOfValue.compareTo(BigDecimal.ZERO) > 0) {
            amountScaled = existingVal.divide(inMultiplesOfValue, 2, RoundingMode.HALF_UP).multiply(inMultiplesOfValue);
        }
        return amountScaled;
    }
    ...
    public static Money roundToMultiplesOf(final Money existingVal, final Integer inMultiplesOf) {
        ...
        BigDecimal inMultiplesOfValue = BigDecimal.valueOf(inMultiplesOf.intValue());
        if (inMultiplesOfValue.compareTo(BigDecimal.ZERO) > 0) {
            amountScaled = amountScaled.divide(inMultiplesOfValue, 2, RoundingMode.HALF_UP).multiply(inMultiplesOfValue);
        }
