...
VALUES ("Housing Type Policy Check", "Table", "Prequalification",
"SELECT
 client_details.housing_type,
  CASE
      ...
      WHEN (${loanProductId} = 6) AND ('${recreditCategorization}' = 'RECREDITO') AND (client_details.housing_type = 'OWN')  THEN 'YELLOW'
  ...
  WHERE mc.id = ${clientId}", "Housing Typed Policy Check", 0, 0, 0);
