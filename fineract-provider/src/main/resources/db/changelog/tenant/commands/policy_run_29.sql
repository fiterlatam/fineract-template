...
-- Present agricultural technical diagnosis (Commcare) Policy Check  --
...
VALUES ("Present agricultural technical diagnosis (Commcare) Policy Check", "Table", "Prequalification",
        "SELECT CASE
       ...
       WHEN (${loanProductId} = 7) AND ${numberOfDocuments} > 0  THEN 'GREEN'
       ELSE 'RED'
  END AS color
  FROM m_client mc WHERE mc.id = ${clientId}", "Present agricultural technical diagnosis (Commcare) Policy Check", 0, 0, 0);
