...
public class AllyCompensationReadWritePlatformServiceImpl implements AllyCompensationReadWritePlatformService {
    ...
    private static final class AllyCompensationSettlementRowMaper implements RowMapper<AllySettlementCompansationCollectionData> {
        ...
        public AllySettlementCompansationCollectionData mapRow(ResultSet rs, int rowNum) throws SQLException {
            return AllySettlementCompansationCollectionData.builder().startDate(rs.getString("start_date")).endDate(rs.getString("end_date"))
                    .clientAllyId(rs.getLong("id")).nit(rs.getString("nit")).companyName(rs.getString("company_name"))
                    .accountNumber(rs.getString("account_number")).accountType(rs.getString("account_type")).bankName(rs.getString("bank_name"))
                    .purchaseAmount(rs.getBigDecimal("purchase_amount")).comissionAmount(rs.getBigDecimal("comission_amount")).vaComissionAmount(rs.getBigDecimal("va_comission_amount"))
                    .netPurchaseAmount(rs.getBigDecimal("net_purchase_amount")).collectionAmount(rs.getBigDecimal("collection_amount")).compensationAmount(rs.getBigDecimal("compensation_amount"))
    ...
    @Override
    public Optional<AllySettlementCompansationCollectionData> getCompensationSettlementByNit(String nit, String startDate, String endDate){
        try {
            final AllyCompensationReadWritePlatformServiceImpl.AllyCompensationSettlementRowMaper rm = new AllyCompensationReadWritePlatformServiceImpl.AllyCompensationSettlementRowMaper();
    ...
    private static final class ClientAllySettlementMaper implements RowMapper<ClientAllySettlementData>{
        public String schema() {
            return " cca.id, cca.nit, \n" +
                    ...
                    "inner join m_ally_purchase_settlement maps on maps.client_ally_id = cca.id \n" +
                    "group by cca.id";
        }
        @Override
        public ClientAllySettlementData mapRow(ResultSet rs, int rowNum) throws SQLException {
            return ClientAllySettlementData.builder().clientAllyId(rs.getLong("id")).nit(rs.getString("nit"))
                    ...
                    .lastClientCollectionJobRun(rs.getString("lastClientCollectionJobRun")).lastClientPurchaseJobRun(rs.getString("lastClientPurchaseJobRun")).build();
        }
    }
