...
public class AllyCompensationReadWritePlatformServiceImpl implements AllyCompensationReadWritePlatformService {
    ...
    private static final class AllyCompensationSettlementRowMaper implements RowMapper<AllySettlementCompansationCollectionData> {
        ...
        public String schema() {
            return " distinct cca.nit,cca.company_name,mpd.payment_bank_cv_id,maps.purchase_amount, commision_amount, amount_va_commision,purchase_settled_comission, \n"
        ...
        @Override
        public AllySettlementCompansationCollectionData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
                throws SQLException {
            return AllySettlementCompansationCollectionData.builder().lastJobsRun(rs.getString("lastJobsRun")).nit(rs.getString("nit"))
                    .build();
        }
    }
