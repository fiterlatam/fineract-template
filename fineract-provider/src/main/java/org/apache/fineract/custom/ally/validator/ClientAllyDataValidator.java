...
public class ClientAllyDataValidator {
    ...
    public ClientAlly validateForCreate(final String json) {
		...
		baseDataValidator.reset().parameter(ClientAllyApiConstants.companyNameParamName).value(companyName).notNull().notExceedingLengthOf(100);
		...
		final Long departmentCodeValueId = this.fromApiJsonHelper.extractLongNamed(ClientAllyApiConstants.departmentCodeValueIdParamName, element);
		...
		final Long liquidationFrequencyCodeValueId = this.fromApiJsonHelper.extractLongNamed(ClientAllyApiConstants.liquidationFrequencyCodeValueIdParamName, element);
		baseDataValidator.reset().parameter(ClientAllyApiConstants.liquidationFrequencyCodeValueIdParamName).value(liquidationFrequencyCodeValueId).notNull();

		final Boolean applyCupoMaxSell = this.fromApiJsonHelper.extractBooleanNamed(ClientAllyApiConstants.applyCupoMaxSellParamName, element);
		baseDataValidator.reset().parameter(ClientAllyApiConstants.applyCupoMaxSellParamName).value(applyCupoMaxSell).notNull();

		final Integer cupoMaxSell = this.fromApiJsonHelper.extractIntegerWithLocaleNamed(ClientAllyApiConstants.cupoMaxSellParamName, element);
		baseDataValidator.reset().parameter(ClientAllyApiConstants.cupoMaxSellParamName).value(cupoMaxSell);

		final BigDecimal settledComission = this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(ClientAllyApiConstants.settledComissionParamName, element);
		...
		final Boolean collectionEnabled = this.fromApiJsonHelper.extractBooleanNamed(ClientAllyApiConstants.collectionEnabledParamName, element);
		baseDataValidator.reset().parameter(ClientAllyApiConstants.collectionEnabledParamName).value(collectionEnabled).notNull();

		final Long bankEntityCodeValueId = this.fromApiJsonHelper.extractLongNamed(ClientAllyApiConstants.bankEntityCodeValueIdParamName, element);
		baseDataValidator.reset().parameter(ClientAllyApiConstants.bankEntityCodeValueIdParamName).value(bankEntityCodeValueId);

		final Long accountTypeCodeValueId = this.fromApiJsonHelper.extractLongNamed(ClientAllyApiConstants.accountTypeCodeValueIdParamName, element);
		...
		final Long taxProfileCodeValueId = this.fromApiJsonHelper.extractLongNamed(ClientAllyApiConstants.taxProfileCodeValueIdParamName, element);
        ...
        return 	ClientAlly.builder()
					...
					.liquidationFrequencyCodeValueId(liquidationFrequencyCodeValueId)
					.applyCupoMaxSell(applyCupoMaxSell)
					.cupoMaxSell(cupoMaxSell)
					.settledComission(settledComission)
					.buyEnabled(buyEnabled)
					.collectionEnabled(collectionEnabled)
					.bankEntityCodeValueId(bankEntityCodeValueId)
					.accountTypeCodeValueId(accountTypeCodeValueId)
					.accountNumber(accountNumber)
					.taxProfileCodeValueId(taxProfileCodeValueId)
    ...
    public ClientAlly validateForUpdate(final String json) {
		...
		ClientAlly ret = ClientAlly.builder()
				.companyName(companyName)
				.nit(nit)
				.nitDigit(nitDigit)
				.address(address)
