 ...
 * under the License.
 */
package org.apache.fineract.custom.infrastructure.bulkimport.populator.clientallypointsofsales;
...
import org.apache.fineract.custom.infrastructure.bulkimport.populator.GenericListSheetPopulator;
...
public class ClientAllyPointsOfSalesWorkbookPopulator extends AbstractWorkbookPopulator {
    ...
    private GenericListSheetPopulator genericListSheetPopulator;
    private List<ClientAllyData> clientsAlliesList;
    private String sheetName;

    public ClientAllyPointsOfSalesWorkbookPopulator(CustomCodeValueReadPlatformService codeValueReadPlatformService, String sheetName,
            List<ClientAllyData> clientsAlliesList) {
        this.codeValueReadPlatformService = codeValueReadPlatformService;
        this.sheetName = sheetName;
        this.clientsAlliesList = clientsAlliesList;

        ClientAllyPointOfSalesTemplatePopulateImportEnum.getOrdered().stream()
                .filter(isCodeValue -> Objects.nonNull(isCodeValue.getCodeValueName())).forEach(obj -> genericCodeValueSheetPopulatorList
                        .add(new GenericCodeValueSheetPopulator(obj.getCodeValueName(), codeValueReadPlatformService)));
    }

    @Override
    public void populate(Workbook workbook, String dateFormat) {
        Sheet staffSheet = workbook.createSheet(this.sheetName);

        this.genericListSheetPopulator = new GenericListSheetPopulator(CustomGlobalEntityType.CLIENT_ALLY.getCode(), clientsAlliesList,
                ClientAllyData.class, "getCompanyName", "getNit");
        genericListSheetPopulator.populate(workbook, dateFormat);
    ...
    private void writeHeaders(Sheet staffSheet, Integer rowIndex) {
        ...
        ClientAllyPointOfSalesTemplatePopulateImportEnum.getOrdered().stream().forEach(obj -> headerSetup(staffSheet, rowHeader, obj));
    }

    private void headerSetup(Sheet staffSheet, Row rowHeader, ClientAllyPointOfSalesTemplatePopulateImportEnum fieldDetailsEnum) {
    ...
    private void setRules(Sheet staffSheet, String dateFormat) {
        ...
        createCustomCellValidatorsDependingOnTheFieldType(ClientAllyPointOfSalesTemplatePopulateImportEnum.COMPANY_NAME_ID,
                CustomGlobalEntityType.CLIENT_ALLY, staffSheet, validationHelper);
    }

    private void createCustomCellValidatorsDependingOnTheFieldType(ClientAllyPointOfSalesTemplatePopulateImportEnum currentCodeValueEnum,
            CustomGlobalEntityType customGlobalEntityType, Sheet staffSheet, DataValidationHelper validationHelper) {
        ...
        setNames(customGlobalEntityType.getCode().replaceAll(" ", "_"), staffSheet, this.clientsAlliesList.size());
        DataValidationConstraint departmentNameConstraint = validationHelper
                .createFormulaListConstraint(customGlobalEntityType.getCode().replaceAll(" ", "_"));
    ...
    private void createGenericCellValidatorsDependingOnTheFieldType(ClientAllyPointOfSalesTemplatePopulateImportEnum currentCodeValueEnum,
