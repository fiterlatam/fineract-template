...
@Path("/v1/customchargehonorariomap")
@Component
@Scope("singleton")
public class CustomChargeHonorarioMapApiResource {
    ...
    @Autowired
    public CustomChargeHonorarioMapApiResource(final DefaultToApiJsonSerializer<CustomChargeHonorarioMapData> toApiJsonSerializer,
    ...
    @Autowired
    private CustomChargeHonorarioMapReadWritePlatformService service;
    ...
    public String retrieveOne(@PathParam("id") @Parameter(description = "id") final Long id, @Context final UriInfo uriInfo) {
        ...
        final CustomChargeHonorarioMapData data = this.service.findById(id);

        final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());

        return this.toApiJsonSerializer.serialize(settings, data, CustomChargeHonorarioMapApiConstants.REQUEST_DATA_PARAMETERS);
    }
    ...
    public String createNewHoliday(@Parameter(hidden = true) final String apiRequestBodyAsJson) {
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().createCustomChargeHonorarioMap().withJson(apiRequestBodyAsJson)
                .build();
    ...
    public String update(@PathParam("id") @Parameter(description = "id") final Long id,
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().updateCustomChargeHonorarioMap(id).withJson(jsonRequestBody)
                .build();
    ...
    public String delete(@PathParam("id") @Parameter(description = "id") final Long id) {
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().deleteCustomChargeHonorarioMap(id).build();
