...
public class PromissoryNoteTemplateOne {
    ...
    private String generate(JsonObject object) {
        ...
        Optional<LoanRepaymentScheduleInstallment> opt = loan.getRepaymentScheduleInstallments()
                .stream()
        ...
        Optional<LoanRepaymentScheduleInstallment> optLast = loan.getRepaymentScheduleInstallments().stream().max(Comparator.comparing(LoanRepaymentScheduleInstallment::getInstallmentNumber));
        ...
        String lastQuotaAmount = optLast.isPresent() ? getNumber(optLast.get().getTotalPrincipalAndInterest(currency).getAmount(), true, true) : "";
        ...
        try {
            ...
            writer.setPageEvent(new HeaderFooterEvent(logo));
            ...
            String bodyText = String.format("""
                            ...
                            i. Renuncio expresamente a los derechos que pudieren conferirme las leyes vigentes o que en el futuro entraren en vigor y que pudieran permitirme cumplir las obligaciones contraídas en este documento en forma distinta a la pactada. Como deudor declaro que estoy plenamente enterado de todas y cada uno de los términos de este pagaré, lo acepto, ratifico y firmo por no saber firmar dejo impresa la huella de mi pulgar derecho firmando como testigo al señor %s, quien es persona capaz e idónea y se identifica con Documento Personal de Identificación (DPI) %s, %s, extendido por el Registro Nacional de las Personas de la República de Guatemala.
                            
                            Lugar y fecha de emisión: Municipio y Departamento de %s, %s de %s, del año dos mil %s.
                            """,
                    clientName, clientDpiText, clientDpiNumber, clientAddress, creditAmountText,
                    creditPurpose, termText, disbursementDate, secondTermText, numberEqualsQuotas + " " + quotaAmount,
                    numberLastQuota, lastQuotaAmount, paymentDay,
                    interestRateText, witnessName, witnessDpiText, witnessDpiNumber,
                    department, date.getDayOfMonth(), date.getMonth().getDisplayName(TextStyle.FULL, new Locale("es")), date.getYear() - 2000
            );
            ...
            // Firmas
            document.add(createSignatureSection(clientName, witnessName));
    ...
    private PdfPTable createSignatureSection(String clientName, String witnessName) {
        ...
        PdfPCell cell1 = new PdfPCell(new Paragraph("F. _________________________________\n\n" +
                clientName.toUpperCase() + "\n\nPromitente deudora o libradora", font));
        cell1.setBorder(Rectangle.NO_BORDER);
        cell1.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(cell1);

        PdfPCell cell2 = new PdfPCell(new Paragraph("F. _________________________________\n\n" +
    ...
    private String getNumber(Number number, boolean parentheses, Boolean uppercase) {
        ...
        String numericValue = bd.stripTrailingZeros().scale() <= 0
