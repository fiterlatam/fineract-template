    ...
    private void assignBaseUrlAndAPIKey() {
        try {
            Collection<ExternalServicesPropertiesData> externalServicesPropertiesDataList = externalServicesPropertiesReadPlatformService
                    .retrieveOne(ExternalServicesConstants.COLLECTION_HOUSE_HISTORY_PROVIDER_SERVICE_NAME);

            baseUrl = externalServicesPropertiesDataList.stream()
                    .filter(obj -> obj.getName().equals(ExternalServicesConstants.COLLECTION_HOUSE_HISTORY_URL))
            ...
            apiKey = externalServicesPropertiesDataList.stream()
                    .filter(obj -> obj.getName().equals(ExternalServicesConstants.COLLECTION_HOUSE_HISTORY_API_KEY))
                    .map(ExternalServicesPropertiesData::getValue).findFirst().orElse(STRING_FALLBACK_API_KEY);

        } catch (Exception e) {
            LOG.error("Error while fetching Collection House History external service properties. Using fallback values.");
