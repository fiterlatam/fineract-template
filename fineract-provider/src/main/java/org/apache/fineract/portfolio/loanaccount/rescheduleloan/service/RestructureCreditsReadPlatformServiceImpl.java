...
public class RestructureCreditsReadPlatformServiceImpl implements RestructureCreditsReadPlatformService {
    ...
    private static final class RestructureCreditsRequestMapper implements RowMapper<RestructureCreditsRequestData> {
        ...
        public RestructureCreditsRequestData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            final LocalDateTime dateRequested = JdbcSupport.getDateTime(rs, "date_requested").toLocalDateTime();
            ZonedDateTime dateApprovedTime = JdbcSupport.getDateTime(rs, "date_approved");
            final LocalDateTime dateApproved = dateApprovedTime != null ? dateApprovedTime.toLocalDateTime() : null;
            final LocalDateTime dateModified = JdbcSupport.getDateTime(rs, "lastmodified_date").toLocalDateTime();
            final LocalDateTime newDisbursementDate = JdbcSupport.getDateTime(rs, "new_disbursement_date").toLocalDateTime();
    ...
    private static final class RestructureCreditsLoanMappingsMapper implements RowMapper<RestructureCreditsLoanMappingData> {
        ...
        RestructureCreditsLoanMappingsMapper() {
            ...
            this.mschema = """
                     rcm.id, mpl.name as productName, rcm.outstanding_balance, rcm.disbursement_date, \
                    ml.maturedon_date as maturity_Date, rcm.status from m_restructure_credits_loans_mapping rcm \
                    inner join m_loan ml on ml.id = rcm.loan_id inner join m_product_loan mpl on mpl.id = ml.product_id\s
                    """;
        }
