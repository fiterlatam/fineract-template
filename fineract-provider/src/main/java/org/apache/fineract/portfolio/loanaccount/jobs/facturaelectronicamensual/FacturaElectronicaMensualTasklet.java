    ...
    public RepeatStatus execute(@NotNull StepContribution contribution, @NotNull ChunkContext chunkContext) throws Exception {
        ...
        if (!businessLocalDate.equals(secondLastDayOfMonth)) {
            final String loanInvoiceQuery = """
                    ...
                    """;
            final List<LoanInvoiceData> loanInvoiceDataList = jdbcTemplate.query(loanInvoiceQuery,
                    (rs, rowNum) -> LoanInvoiceData.builder().loanId(rs.getLong("loanId"))
                            .clientId(rs.getLong("clientId"))
                            .clientLegalForm(rs.getInt("clientLegalForm"))
                            .clientDisplayName(rs.getString("clientDisplayName"))
                            .clientLastName(rs.getString("clientLastName"))
                            .clientEmailAddress(rs.getString("clientEmailAddress"))
                            .overdueSinceDate(JdbcSupport.getLocalDate(rs, "overdueSinceDate"))
                            .daysInArrears(rs.getInt("daysInArrears"))
                            .productTypeId(rs.getLong("productTypeId"))
                            .productTypeName(rs.getString("productTypeName"))
                            .clientIdNumber(rs.getString("clientIdNumber"))
                            .billingPrefix(rs.getString("billingPrefix"))
                            .billingResolutionNumber(rs.getString("billingResolutionNumber"))
                            .rangeStartNumber(JdbcSupport.getLong(rs, "rangeStartNumber"))
                            .rangeEndNumber(JdbcSupport.getLong(rs, "rangeEndNumber"))
                            ...
                            .lastDebitNoteNumber(JdbcSupport.getLong(rs, "lastDebitNoteNumber"))
                            .loanProductName(rs.getString("loanProductName"))
                            .companyNIT(rs.getString("companyNIT"))
                            .companyDeptCode(rs.getString("companyDeptCode"))
                            .companyDeptName(rs.getString("companyDeptName"))
                            .companyMunCode(rs.getString("companyMunCode"))
                            .companyMunName(rs.getString("companyMunName"))
                            .companyCityCode(rs.getString("companyCityCode"))
                            .companyCityName(rs.getString("companyCityName"))
                            .companyAddress(rs.getString("companyAddress"))
                            .companyTelephone(rs.getString("companyTelephone"))
                            .clientCedula(rs.getString("clientCedula"))
                            .clientAddress(rs.getString("clientAddress"))
                            .clientCityCode(rs.getString("clientCityCode"))
                            .clientCityName(rs.getString("clientCityName"))
            ...
            final List<LoanInvoiceData> groupedLoanInvoices = loanInvoiceDataList.stream()
                    ...
                    .collect(Collectors.groupingBy(
                            li1 -> Arrays.asList(li1.getClientId(), li1.getProductTypeName()),
                            Collectors.collectingAndThen(
                                    Collectors.toList(),
                                    list -> {
                                        final BigDecimal outstandingPrincipal = list.stream().map(LoanInvoiceData::getOutstandingPrincipal).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal currentInterest = list.stream().map(LoanInvoiceData::getCurrentInterest).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal overdueInterest = list.stream().map(LoanInvoiceData::getOverdueInterest).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal outstandingPenalty = list.stream().map(LoanInvoiceData::getOutstandingPenalty).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal outstandingMandatoryInsurance = list.stream().map(LoanInvoiceData::getOutstandingMandatoryInsurance).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal outstandingVoluntaryInsurance = list.stream().map(LoanInvoiceData::getOutstandingVoluntaryInsurance).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal outstandingAval = list.stream().map(LoanInvoiceData::getOutstandingAval).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal outstandingHonorarios = list.stream().map(LoanInvoiceData::getOutstandingHonorarios).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final BigDecimal totalOutstanding = list.stream().map(LoanInvoiceData::getTotalOutstanding).reduce(BigDecimal.ZERO, BigDecimal::add);
                                        final Integer loansCount = list.size();
                                        Integer itemsCounts = 0;
                                        for(final LoanInvoiceData loanInvoiceData : list) {
                                            if(loanInvoiceData.getOutstandingPrincipal().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getCurrentInterest().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getOverdueInterest().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getOutstandingPenalty().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getOutstandingMandatoryInsurance().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getOutstandingVoluntaryInsurance().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getOutstandingAval().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                            if(loanInvoiceData.getOutstandingHonorarios().compareTo(BigDecimal.ZERO) > 0) {
                                                itemsCounts++;
                                            }
                                        }
                                        ...
                                        return  LoanInvoiceData.builder()
                                                .clientIdNumber(loanInvoiceData.getClientIdNumber())
                                                .clientDisplayName(loanInvoiceData.getClientDisplayName())
                                                .clientLastName(loanInvoiceData.getClientLastName())
                                                .clientLegalForm(loanInvoiceData.getClientLegalForm())
                                                .clientId(loanInvoiceData.getClientId())
                                                .clientEmailAddress(loanInvoiceData.getClientEmailAddress())
                                                .loanId(loanInvoiceData.getLoanId())
                                                ...
                                                .daysInArrears(loanInvoiceData.getDaysInArrears())
                                                ...
                                                .outstandingMandatoryInsurance(outstandingMandatoryInsurance)
                                                .outstandingVoluntaryInsurance(outstandingVoluntaryInsurance)
                                                .outstandingAval(outstandingAval)
                                                .outstandingHonorarios(outstandingHonorarios)
                                                ...
                                                .loanProductName(loanInvoiceData.getLoanProductName())
                                                .companyNIT(loanInvoiceData.getCompanyNIT())
                                                ...
                                                .clientCedula(loanInvoiceData.getClientCedula())
                                                .clientAddress(loanInvoiceData.getClientAddress())
                                                .clientCityCode(loanInvoiceData.getClientCityCode())
                                                .clientCityName(loanInvoiceData.getClientCityName())
                                                .clientTelephone(loanInvoiceData.getClientTelephone())
                                                .build();
                                    }
                            )
                    ))
