    ...
    public RepeatStatus execute(@NotNull StepContribution contribution, @NotNull ChunkContext chunkContext) throws Exception {
        ...
        if (businessLocalDate.equals(secondLastDayOfMonth) || enableMonthlyInvoiceGenerationOnJobTrigger) {
            ...
            final List<LoanDocumentData> loanInvoiceDataList = this.jdbcTemplate.query(invoiceQuery, loanInvoiceMapper,
                    LoanTransactionType.REPAYMENT.getValue(), LoanTransactionType.REPAYMENT.getValue(), secondLastDayOfMonth);
            ...
            final List<LoanDocumentData> loanCreditNoteDataList = this.jdbcTemplate.query(invoiceQuery, loanInvoiceMapper,
                    LoanTransactionType.CREDIT_NOTE.getValue(), LoanTransactionType.CREDIT_NOTE.getValue(), secondLastDayOfMonth);
            final String creditNoteTransactionIds = loanCreditNoteDataList.stream().map(LoanDocumentData::getTransactionIds)
                    .collect(Collectors.joining(","));
            ...
            if (StringUtils.isNotBlank(invoiceTransactionIds) && StringUtils.isNotBlank(creditNoteTransactionIds)) {
                transactionIds = invoiceTransactionIds + "," + creditNoteTransactionIds;
            } else if (StringUtils.isNotBlank(invoiceTransactionIds)) {
                transactionIds = invoiceTransactionIds;
            } else if (StringUtils.isNotBlank(creditNoteTransactionIds)) {
                transactionIds = creditNoteTransactionIds;
            }
