...
public class FacturaElectronicaMensualPoster {
    ...
    public void processInvoice() throws JobExecutionException {
        ...
        try {
            log.info("Processing invoices for client with IDs size: {}", clientIds.size());
            if (this.clientIds == null || this.clientIds.isEmpty()) {
                log.warn("No client IDs provided for Factura Electronica Mensual processing.");
                return;
            }
            final long startTime = System.currentTimeMillis();
            List<Long> loanIds = this.loanReadPlatformService.retrieveLoanIdsForInvoiceGenerationByClientIds(this.clientIds,
                    secondLastDayOfMonth);
            final long endTime = System.currentTimeMillis();
            log.info("Retrieved {} loan IDs for invoice generation in {} ms", loanIds.size(), (endTime - startTime));
            ...
            log.info("Retrieved {} loan invoice data in {} ms", loanInvoiceDataList.size(), (retrievalEndTime - endTime));
            ...
            log.info("Grouped {} loan invoices into {} client/product type combinations", loanInvoiceDataList.size(),
                    loanInvoicesMap.size());
            ...
            log.info("Processed invoices for client with IDs size: {}", this.clientIds.size());
        ...
        log.info("Factura Electronica Mensual processing completed successfully for client IDs size: {}", this.clientIds.size());
    }
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.collectingAndThen(Collectors.toList(), list -> {
                            ...
                            final String mandatoryInsuranceName = list.stream().map(LoanDocumentData::getMandatoryInsuranceName)
                                    .filter(Objects::nonNull).findFirst().orElse(null);
                            final String voluntaryInsuranceNIT = list.stream().map(LoanDocumentData::getVoluntaryInsuranceNIT)
                                    .filter(Objects::nonNull).findFirst().orElse(null);
                            final String mandatoryInsuranceNIT = list.stream().map(LoanDocumentData::getMandatoryInsuranceNIT)
                                    .filter(Objects::nonNull).findFirst().orElse(null);
