...
@Service
@CommandType(entity = BankChequeApiConstants.BANK_CHECK_RESOURCE_NAME, action = BankChequeApiConstants.CHECK_ACTION_AUTHORIZE_ISSUANCE)
@RequiredArgsConstructor
public class AuthorizeChequeIssuanceCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(JsonCommand command) {
        try {
            return this.chequeWritePlatformService.authorizeChequeIssuance(command);
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve, "bankcheques",
                    "authorize cheque issuance");
            return CommandProcessingResult.empty();
        } catch (final PersistenceException dve) {
            Throwable throwable = ExceptionUtils.getRootCause(dve.getCause());
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, throwable, dve, "bankcheques", "authorize cheque issuance");
            return CommandProcessingResult.empty();
        }
    }
}
