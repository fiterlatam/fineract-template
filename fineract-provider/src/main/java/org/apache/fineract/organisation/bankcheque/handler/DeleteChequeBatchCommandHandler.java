...
@Service
@CommandType(entity = BankChequeApiConstants.BANK_CHECK_RESOURCE_NAME, action = BankChequeApiConstants.CHECK_ACTION_DELETE)
@RequiredArgsConstructor
@Slf4j
public class DeleteChequeBatchCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(JsonCommand command) {
        try {
            return this.chequeWritePlatformService.deleteBatch(command.entityId(), command);
            ...
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, throwable, dve, "bankcheques", "delete cheque batch");
            return CommandProcessingResult.empty();
        }
    }
