...
@Service
@CommandType(entity = BankChequeApiConstants.BANK_CHECK_RESOURCE_NAME, action = BankChequeApiConstants.CHECK_ACTION_PRINT_BATCH)
@RequiredArgsConstructor
public class PrintChequeBatchCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(JsonCommand command) {
        try {
            return this.chequeWritePlatformService.printChequeBatches(command);
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve, "bankcheques",
                    "print cheque batches");
            return CommandProcessingResult.empty();
        } catch (final PersistenceException dve) {
            Throwable throwable = ExceptionUtils.getRootCause(dve.getCause());
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, throwable, dve, "bankcheques", "print cheque batches");
            return CommandProcessingResult.empty();
        }
    }
}
