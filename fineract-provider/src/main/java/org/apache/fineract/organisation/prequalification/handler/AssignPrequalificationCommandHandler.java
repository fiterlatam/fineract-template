...
@CommandType(entity = "PREQUALIFICATIONS", action = "ASSIGN")
public class AssignPrequalificationCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        try {
            return this.prequalificationWritePlatformService.assignPrequalification(command.entityId(), command);
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve,
                    "error.msg.prequalification.assignment.failed", "Failed to assign Prequalification");
