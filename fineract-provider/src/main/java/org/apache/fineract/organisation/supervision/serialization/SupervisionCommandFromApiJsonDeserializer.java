 ...
 * under the License.
 */
package org.apache.fineract.organisation.supervision.serialization;
 ...
 */
@Component
public class SupervisionCommandFromApiJsonDeserializer {

    /**
     * The parameters supported for this command.
     */
    private final Set<String> supportedParameters = SupervisionConstants.SupervisionSupportedParameters.getAllValues();

    private final FromJsonHelper fromApiJsonHelper;

    @Autowired
    public SupervisionCommandFromApiJsonDeserializer(final FromJsonHelper fromApiJsonHelper) {
    ...
    public void validateForCreate(String json) {
        ...
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors)
                .resource(SupervisionConstants.SUPERVISION_RESOURCE_NAME);

        final JsonElement element = this.fromApiJsonHelper.parse(json);

        final String name = this.fromApiJsonHelper.extractStringNamed(SupervisionConstants.SupervisionSupportedParameters.NAME.getValue(),
                element);
        baseDataValidator.reset().parameter(SupervisionConstants.SupervisionSupportedParameters.NAME.getValue()).value(name).notBlank()
                .notExceedingLengthOf(60);

        if (this.fromApiJsonHelper.parameterExists(SupervisionConstants.SupervisionSupportedParameters.OFFICE_PARENT_ID.getValue(),
                element)) {
