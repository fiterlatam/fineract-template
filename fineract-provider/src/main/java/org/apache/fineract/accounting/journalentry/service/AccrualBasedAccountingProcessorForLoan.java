...
public class AccrualBasedAccountingProcessorForLoan implements AccountingProcessorForLoan {
    ...
    private void createJournalEntriesForVatAccruals(LoanDTO loanDTO, LoanTransactionDTO loanTransactionDTO, Office office) {
        ...
        if (loanTransactionDTO.isGeneratedTransactionFromRepayment()) {
            ...
            if (loanTransactionDTO.getVatOnPenaltyCharges() != null
                    && loanTransactionDTO.getVatOnPenaltyCharges().compareTo(BigDecimal.ZERO) > 0) {
                creditTypeAccountForRepaymentAccrual = AccrualAccountsForLoan.PENALTIES_RECEIVABLE.getValue();
            }
        } else if (loanTransactionDTO.getVatOnPenaltyCharges() != null
                && loanTransactionDTO.getVatOnPenaltyCharges().compareTo(BigDecimal.ZERO) > 0) {
            // accrual VAT penalty
