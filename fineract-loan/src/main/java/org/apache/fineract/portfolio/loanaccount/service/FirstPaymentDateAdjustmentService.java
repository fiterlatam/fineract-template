...
/**
 * Service to handle automatic adjustment of first payment dates when disbursement date
 * is after the configured first repayment date.
 * 
 * This service leverages the existing RevolvingLoanUtil for consistent date normalization
 * and follows the same business rules for repayment date calculations.
 */
@Service
...
public class FirstPaymentDateAdjustmentService {
    ...
    /**
     * Checks if the first payment date needs adjustment and adjusts it if necessary.
     * This method is called during loan disbursement to ensure the first repayment date
     * is always after the disbursement date.
     *
    ...
    public boolean adjustFirstPaymentDateIfNeeded(Loan loan, LocalDate disbursementDate) {
        LocalDate currentFirstRepaymentDate = loan.getExpectedFirstRepaymentOnDate();
        
        if (currentFirstRepaymentDate == null) {
            log.debug("No first repayment date configured for loan {}, skipping adjustment", loan.getId());
            return false;
        }

        // Check if disbursement date is after the first repayment date
        if (DateUtils.isAfter(disbursementDate, currentFirstRepaymentDate)) {
            log.info("Disbursement date {} is after first repayment date {} for loan {}. Adjusting first payment date.",
                    disbursementDate, currentFirstRepaymentDate, loan.getId());
    ...
    /**
     * Calculates the adjusted first payment date based on the loan product frequency
    ...
    private LocalDate calculateAdjustedFirstPaymentDate(Loan loan, LocalDate disbursementDate) {
        ...
        LocalDate adjustedDate = switch (frequency) {
            ...
            case INVALID, WHOLE_TERM -> {
                log.warn("Unsupported frequency type {} for loan {}, using default monthly calculation", 
                        frequency, loan.getId());
    ...
    /**
     * Calculates the first payment date for monthly frequency loans.
     * Uses the same logic as RevolvingLoanUtil for consistency.
     *
     * @param disbursementDate the disbursement date
     * @param repayEvery the repayment frequency (number of months)
    ...
    /**
     * Normalizes a repayment date for revolving loans to use standard repayment days (1, 10, 20).
     * This method uses the same logic as RevolvingLoanUtil for consistency.
     *
     * @param repaymentDate the original repayment date
     ...
     *
     * @param loan the loan
     * @param adjustedFirstPaymentDate the adjusted first payment date
    ...
    public boolean isValidFirstPaymentDate(Loan loan, LocalDate adjustedFirstPaymentDate) {
        ...
        if (!DateUtils.isAfter(adjustedFirstPaymentDate, disbursementDate)) {
            log.warn("Adjusted first payment date {} is not after disbursement date {} for loan {}", 
                    adjustedFirstPaymentDate, disbursementDate, loan.getId());
            return false;
        }
