    ...
    public void validatePairOfOrderAndPaymentAllocationType(List<Pair<Integer, PaymentAllocationType>> rules) {
        if (rules.size() != 24) {
            raiseValidationError("advanced-payment-strategy.each_payment_allocation_order.must.contain.12.entries",
                    "Each provided payment allocation must contain exactly 24 allocation rules, but " + rules.size() + " were provided");
        }
        ...
        if (!Arrays.equals(IntStream.rangeClosed(1, 24).boxed().toArray(), rules.stream().map(Pair::getLeft).toArray())) {
            raiseValidationError("advanced-payment-strategy.invalid.order", "The provided orders must be between 1 and 24");
        }
    }
